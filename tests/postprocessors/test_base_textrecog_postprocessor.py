...
class TestBaseTextRecogPostprocessor(TestCase):
    def test_init(self):
        ...
        self.assertListEqual(
            base_postprocessor.ignore_indexes,
        ...
        # test ignore_chars
        with self.assertRaisesRegex(AssertionError, "ignore_chars must be list of str"):
            base_postprocessor = BaseTextRecogPostprocessor(
                dict_cfg, ignore_chars=[1, 2, 3]
            )
        with self.assertWarnsRegex(Warning, "M does not exist in the dictionary"):
            base_postprocessor = BaseTextRecogPostprocessor(
        ...
        with self.assertWarnsRegex(Warning, "M does not exist in the dictionary"):
            base_postprocessor = BaseTextRecogPostprocessor(
                Dictionary(chars, with_unknown=True, unknown_token=None),
                ignore_chars=["M"],
            )

        with self.assertWarnsRegex(Warning, "M does not exist in the dictionary"):
            base_postprocessor = BaseTextRecogPostprocessor(
                Dictionary(chars, with_unknown=True), ignore_chars=["M"]
            )

        with self.assertWarnsRegex(Warning, "unknown does not exist in the dictionary"):
            base_postprocessor = BaseTextRecogPostprocessor(
                Dictionary(chars, with_unknown=False), ignore_chars=["unknown"]
            )
