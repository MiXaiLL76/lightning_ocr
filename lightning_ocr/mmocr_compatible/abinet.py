    ...
    for key in list(state_dict):
            ...
            if "encoder.transformer" in key:
                if "attentions" in key:
                    if ".attn." in key:
                        ...
                        if idx == "0":
                            new_key = key.replace(f"transformer.{idx}.attentions.0.attn", "encoder_layer.self_attn")
                        else:
                            new_key = key.replace(f"transformer.{idx}.attentions.0.attn", f"transformer.layers.{int(idx)-1}.self_attn")
            ...
            if ("transformer" in key):
                if ("ffns" in key) and ("layers" in key):
                    ...
                    if "0.0" in key:
                        ...
                        if idx == "0":
                            new_key = key.replace(f"transformer.{idx}.ffns.0.layers.0.0", "encoder_layer.linear1")
                        else:
                            new_key = key.replace(f"transformer.{idx}.ffns.0.layers.0.0", f"transformer.layers.{int(idx)-1}.linear1")
                    else:
                        idx = key.split(".")[2]
                        idx2 = key.split(".")[6]
                        
                        if idx == "0":
                            new_key = key.replace(f"transformer.{idx}.ffns.0.layers.{idx2}", "encoder_layer.linear2")
                        else:
                            new_key = key.replace(f"transformer.{idx}.ffns.0.layers.{idx2}", f"transformer.layers.{int(idx)-1}.linear2")
                    ...
                    idx2 = key.split(".")[4]
                    if idx == "0":
                        new_key = key.replace(f"transformer.{idx}.norms.{idx2}", f"encoder_layer.norm{int(idx2)+1}")
                    else:
                        new_key = key.replace(f"transformer.{idx}.norms.{idx2}", f"transformer.layers.{int(idx)-1}.norm{int(idx2)+1}")
                    # print(new_key)
        ...
        if key.startswith("decoder"):
            ...
            if "k_encoder" in key:
                ...
                new_key = key.replace(f"k_encoder.{idx}.{idx_type}", f"k_encoder.{idx}.{new_idx_type}")
                ...
                new_key = key.replace(f"k_decoder.{idx}.1.{idx_type}", f"k_decoder.{idx}.{new_idx_type}")
