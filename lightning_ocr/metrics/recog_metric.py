...
from lightning_ocr.metrics.base import BaseMetric
...
class WordMetric(BaseMetric):
    ...
    def __init__(
        ...
        assert set(mode).issubset({"exact", "ignore_case", "ignore_case_symbol"})
        self.mode = set(mode)
    ...
    def process(self, data_batch: Sequence[Dict], data_samples: Sequence[Dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            pred_text = data_sample.get("pred_text")
            gt_text = data_sample.get("gt_text")
    ...
    def compute_metrics(self, results: Sequence[Dict]) -> Dict:
        ...
        if "ignore_case" in self.mode:
            ...
            eval_res["word_acc_ignore_case"] = (
                1.0 * match_ignore_case_num / (eps + gt_word_num)
            )
        if "ignore_case_symbol" in self.mode:
            match_ignore_case_symbol_num = [
            ...
            eval_res["word_acc_ignore_case_symbol"] = (
                1.0 * match_ignore_case_symbol_num / (eps + gt_word_num)
            )
