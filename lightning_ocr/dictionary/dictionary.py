...
class Dictionary:
    ...
    Args:
        dict_list (list[str]): Characters list
    ...
    def fast_tokenizer(self):
        ...
        assert self.with_unknown, "fast_tokenizer need unknown_token"
        assert self.with_padding, "fast_tokenizer need padding_token"
        assert self.with_end, "fast_tokenizer need end_token"
        assert self.with_start, "fast_tokenizer need start_token"

        bad_keys = [self.start_end_token]
        base_vocab = {
            self.start_token: 0,
        ...
        for key in self._char2idx.keys():
            if key in base_vocab:
                continue

            if key not in bad_keys:
                base_vocab[key] = len(base_vocab)

        bpe_kwargs = {
            "vocab": base_vocab,
            "merges": [],
        }
        bpe_kwargs["unk_token"] = self.unknown_token
        ...
        if self.with_padding:
            tokenizer.enable_padding(
                pad_id=base_vocab[self.padding_token], pad_token=self.padding_token
            )
